...
func slaveRecursiveBind(from, to string) error {
	_, err := exec.CombinedOutput(goexec.Command(
		...
		// existing dummy-autofuse-csi mounts inside of the autofs root.
		"--rbind",

		// We expect the autofs root in /dummy to be already marked
		...
		// this would then mean attempting to unmount autofs-dummyfs mounts
		// in the rest of the bindmounts (used by other Pods on the node
		// that also use dummy-autofuse-csi), which is not desirable of course.
		"--make-slave",
	))
