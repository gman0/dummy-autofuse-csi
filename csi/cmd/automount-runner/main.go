...
func main() {
	...
	if *version {
		fmt.Println("automount-runner for dummy-autofuse-csi plugin version", dummyversion.FullVersion())
		os.Exit(0)
	}

	// Initialize and run automount-runner.

	log.Infof("automount-runner for dummy-autofuse-csi plugin version %s", dummyversion.FullVersion())
